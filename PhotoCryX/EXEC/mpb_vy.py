...
gdsII_file = P.get("GDS_FILE", "valley.gds")
LAYER = 1
si_zmin = 0
si_zmax = 0.22
scale_factor = 1.0
...
BZ_POINTS = {
    ...
    3: (mp.Vector3(2/3, 1/3), "K'"),
}

# Read KPATH from mpb.in
KPATH = P.get("KPATH", "0 2 1 0 3 2 0")
...
# ============================================================
#                 BAND STRUCTURE both TE and TM in THz
# ============================================================
...
def plot_both_norm(tm_freqs, te_freqs, filename):
    ...
    ax.text(100, 0.04, 'TM', color='blue', size=15)
    ax.text(100, 0.14, 'TE', color='red', size=15)
    ticks = np.linspace(0, len(tm_freqs)-1, len(k_labels))
    ax.set_xticks(ticks)
    ax.set_xticklabels(k_labels, size=14)
...
md = mpb.MPBData(rectify=True, resolution=256, periods=2)
