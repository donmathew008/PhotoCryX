...
def run_code():
    ...
    geometry = [mp.Cylinder(r, material=mp.Medium(epsilon=eps), center=mp.cartesian_to_lattice(mp.Vector3(0,s),geometry_lattice)),
            	mp.Cylinder(r, material=mp.Medium(epsilon=eps), center=mp.cartesian_to_lattice(mp.Vector3(0,-s),geometry_lattice)),
            	mp.Cylinder(r, material=mp.Medium(epsilon=eps), center=mp.cartesian_to_lattice(mp.Vector3(a/2,s/2),geometry_lattice)),
            	mp.Cylinder(r, material=mp.Medium(epsilon=eps), center=mp.cartesian_to_lattice(mp.Vector3(a/2,-s/2),geometry_lattice)),
            	mp.Cylinder(r, material=mp.Medium(epsilon=eps), center=mp.cartesian_to_lattice(mp.Vector3(-a/2,s/2),geometry_lattice)),   
            	mp.Cylinder(r, material=mp.Medium(epsilon=eps), center=mp.cartesian_to_lattice(mp.Vector3(-a/2,-s/2),geometry_lattice))]
...
#plot epsilon function
    ...
    eps = mpb.MPBData(rectify=True, periods=2, resolution=512).convert(ms.get_epsilon())
...
# ============================================================
    ...
    for gap in te_gaps:
        if gap[0] > 1:
            ax.fill_between(x, gap[1], gap[2], color='red', alpha=0.2)
    ax.text(48, 0.05, 'TM', color='blue', size=15)
    ax.text(48, 0.15, 'TE', color='red', size=15)
...
#    ax.text(12, 0.04, 'TM', color='blue', size=15)
    ...
    md = mpb.MPBData(rectify=True, periods=2, resolution=512)
...
root.title("Band Structure of 2D Honeycomb Photonic Crystal using MPB")
...
label3 = ttk.Label(root, text="r/a ratio: \n (default 0.14)", font=("bitstream charter", 11))
...
label5 = ttk.Label(root, text="Dielectric constant of dielectric cylinder \n (default 11.7)", font=("bitstream charter", 11))
